...
def fetch_gaode_tiles(min_lon, max_lon, min_lat, max_lat, zoom):
    ...
    print(f"Fetching Map A tiles from x:{min_x}-{max_x}, y:{min_y}-{max_y} at zoom {zoom}")
...
dining_df = pd.read_csv(os.path.join(data_dir, "amap_餐饮服务.csv"))
transport_df = pd.read_csv(os.path.join(data_dir, "amap_交通设施服务.csv"))
public_df = pd.read_csv(os.path.join(data_dir, "amap_公共设施.csv"))
...
for df in [dining_df, transport_df, public_df]:
    convert_df(df, 'lon', 'lat')
...
supply_dfs = []
for df in [dining_df, transport_df, public_df, ctrip_df]:
    df_clean = df[['wgs84_lng', 'wgs84_lat']].dropna()
    supply_dfs.append(df_clean)
supply_all = pd.concat(supply_dfs, ignore_index=True)
...
def plot_supply_map(ax_A, bounds_deg, is_zoom=False):
    ...
    # Legend and colorbar inside the plot!
    ...
    cb.set_label('log(1+鏈嶅姟 POI 鏁?', fontsize=10, weight='bold')
    cax.xaxis.set_ticks_position('top')
    cax.xaxis.set_label_position('top')
    
    # Overlay Anchors
    ax_A.scatter(anchors_gdf_proj.geometry.x, anchors_gdf_proj.geometry.y, s=40, color='white', edgecolor='black', linewidth=1, zorder=5, label='鏍稿績鏂囨梾閿氱偣')
...
plot_supply_map(ax_A_full, (126.48, 126.95, 45.64, 45.84), is_zoom=False)
fig_A_full.tight_layout()
fig_A_full.savefig(os.path.join(out_dir, "fig_gis_A_supply_full.png"), dpi=300)
plt.close(fig_A_full)

fig_A_core, ax_A_core = plt.subplots(figsize=(10, 7), dpi=300)
plot_supply_map(ax_A_core, (126.52, 126.72, 45.72, 45.82), is_zoom=True)
fig_A_core.tight_layout()
fig_A_core.savefig(os.path.join(out_dir, "fig_gis_A_supply_core.png"), dpi=300)
