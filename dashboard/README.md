...
基于 Streamlit 的单页交互应用，将研究报告的静态图表转为可交互展示，数据仅依赖已入库的 `V30` 聚合结果（**不含任何原始评论/笔记，可直接公开**）。
...
诊断分类逻辑忠实复现研究报告 3.2.6 的四类诊断信号（高需求—低供给 / 高需求—高供给—高风险 / 低需求—高风险 / 低需求—高供给），判定规则见 `dashboard_data.py::classify_anchor`。
...
- 数据源：`analysis/V30_Multi_Source_Fusion_R2/*.csv`（20 锚点聚合统计），指标由 `src/engines/metrics_engine.py` 计算。
